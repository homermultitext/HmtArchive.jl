...
While it is straightforward to find an *Iliad* passage and *scholia* commenting on it with the current version of `HmtArchive`, this common task should be easier in a future release of the package with higher-order functions minimizing the number of steps required.
:::
...
::: {.callout-note title="This will soon be easier"}
A forthcoming version of the `CitableAnnotations` package includes `annotates` and `annotatedby` functions that can reduce the filtering in the following section to a single function call. 
:::


The data in a `CitableCommentary` are organized as a Vector of pairs of URNs, with the first URN identifying the annotator, and the second URN the annotated object. We'll get the results we want by filtering on the second part of each pair, and keeping the first part (the *scholion* URN).
...
## Utilities for working with the contents of *scholia*

The `HmtArchive.Analysis` module includes several utility functions for working with the contents of *scholia* identified by URN.

To extract `CitablePassage`s from a corpus, use the `scholion` function:
...
You can separately extract the text of the lemma (if any) and the comment: 
...
```{julia}
scholia_list = map(scholia) do sch
    "- " * scholion_text_md(sch, normalizedtexts)
end
```

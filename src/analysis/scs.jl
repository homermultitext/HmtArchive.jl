...
"""
@memoize function scs1(x, y)
    if isempty(x)
        return y
    elseif isempty(y)
        return x
    elseif x[1] == y[1]
        return append!([x[1]], scs1(x[2:end], y[2:end]))
    elseif length(scs1(x, y[2:end])) <= length(scs1(x[2:end], y))
        return append!([y[1]], scs1(x, y[2:end]))
    else
        return append!([x[1]], scs1(x[2:end], y))
    end
end
